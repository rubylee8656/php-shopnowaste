...
if ($totalRows) {
    ...
    // );
    $sql = sprintf(
        "SELECT `shop_list`.*,`shop_address_city`.`shop_city`,`shop_address_area`.`shop_area` FROM shop_list JOIN `shop_address_city` ON `shop_list`.`shop_address_city_sid` = `shop_address_city`.`sid` JOIN `shop_address_area` ON `shop_list`.`shop_address_area_sid` = `shop_address_area`.`sid` ORDER BY sid DESC LIMIT %s, %s",
        ($page - 1) * $perPage,
        $perPage
    );
...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col">
            <form action="03-shop-search.php" method="POST">
                <input type="text" id="search" name="search" placeholder="以縣市搜尋">
                <input type="text" id="search" name="search" placeholder="以地區搜尋">
                <a href="03-shop-search.php"><button type="submit">搜尋</button></a>
            </form>
        </div>
    </div>
    <br>
